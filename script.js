...
(function () {
  ...
  function addToCart(product, size, color, qty) {
    ...
    if (existing) { existing.qty += qty; }
      ...
      items.push({
        ...
        price: effPrice(product), image: product.images[0],
  ...
  function productCardHTML(p) {
    ...
    return '<a class="product-card" href="product.html?id=' + encodeURIComponent(p.id) + '">' +
      '<div class="img-wrap">' +
        '<img class="' + (p.inStock ? "" : "oos") + '" src="' + p.images[0] + '" alt="' + esc(p.name) + '" loading="lazy">' +
        badges +
      '</div>' +
      '<div class="info">' +
  ...
  function initProduct() {
    ...
    loadProducts(function (err, products) {
      ...
      function render() {
        ...
        var thumbHTML = p.images.map(function (img, i) {
        ...
        mount.innerHTML =
          ...
          '<div class="pdp">' +
            ...
            '<div class="pdp-main"><img src="' + p.images[state.img] + '" alt="' + esc(p.name) + '"></div>' +
            '<div class="pdp-info">' +
        ...
        // wire interactions
        $all("[data-color]", mount).forEach(function (b) { b.addEventListener("click", function () { state.color = b.getAttribute("data-color"); render(); }); });
  ...
  function initCheckout() {
    ...
    mount.innerHTML =
      '<div class="checkout-layout">' +
        '<form id="checkoutForm">' +
          '<div class="form-section"><h2>CONTACT</h2><div class="field"><label for="email">Email *</label><input id="email" type="email" required placeholder="you@example.com"></div></div>' +
          '<div class="form-section"><h2>SHIPPING ADDRESS</h2>' +
            '<div class="field"><label for="name">Full name *</label><input id="name" required placeholder="Jordan Avery"></div>' +
            '<div class="field"><label for="address">Address *</label><input id="address" required placeholder="123 Example Street"></div>' +
            '<div class="field-row"><div class="field"><label for="city">City *</label><input id="city" required placeholder="Portland"></div><div class="field"><label for="zip">ZIP / Postal *</label><input id="zip" required placeholder="97201"></div></div>' +
            '<div class="field"><label for="country">Country</label><input id="country" value="India"></div>' +
          '</div>' +
          '<div class="form-section"><h2>PAYMENT</h2><div class="payment-note"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Payment is collected securely at the next step. This is a demo store — no real payment will be processed.</div></div>' +
        '</form>' +
    ...
    $("#placeOrderBtn").addEventListener("click", function (e) {
      ...
      var order = {
        ...
        city: $("#city").value, zip: $("#zip").value, country: $("#country").value
      };
  ...
  function initConfirmation() {
    ...
    mount.innerHTML =
      '<div class="confirm-head"><span class="confirm-check"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6 9 17l-5-5" stroke-linecap="round" stroke-linejoin="round"/></svg></span><p class="eyebrow">ORDER CONFIRMED</p><h1>THANK YOU</h1><p>Your order has been placed. A confirmation email is on its way to <strong>' + esc(order.email) + '</strong>.</p></div>' +
      '<div class="confirm-meta"><div class="cell"><p class="label">ORDER NUMBER</p><p class="val big">' + order.number + '</p></div><div class="cell"><p class="label">DATE</p><p class="val">' + date + '</p></div><div class="cell"><p class="label">TOTAL</p><p class="val">' + money(order.total) + '</p></div></div>' +
      '<div class="confirm-card"><div class="head"><h2>SUMMARY</h2></div>' + itemsHTML + '<div class="totals"><div class="row"><span>Subtotal</span><span>' + money(order.subtotal) + '</span></div><div class="row"><span>Shipping</span><span>' + (order.shipping === 0 ? "FREE" : money(order.shipping)) + '</span></div><div class="row total"><span>Total</span><span>' + money(order.total) + '</span></div></div></div>' +
      '<div class="confirm-ship"><h2>SHIPPING TO</h2><p>' + esc(order.name) + '<br>' + esc(order.address) + '<br>' + esc(order.city) + ', ' + esc(order.zip) + '<br>' + esc(order.country) + '</p></div>' +
      '<div class="confirm-actions"><a href="products.html" class="btn btn-primary">CONTINUE SHOPPING</a><a href="index.html" class="btn btn-outline">BACK TO HOME</a></div>';
  }
  ...
  function initAccount() {
    ...
    setTimeout(function () {
      if (window.RRG_AUTH && window.RRG_AUTH.getUser()) {
